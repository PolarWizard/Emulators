...
void Intel_8080::IRQ(uint16_t addr) {
	if (interrupt) {
		interrupt = 0;
		RAM[--SP] = (PC) >> 8;
		RAM[--SP] = (PC);
...
void Intel_8080::loadRAM() {
	ifstream myfile;
	myfile.open("C:/Users/domin/OneDrive/Desktop/invaders.concatenated", ios::binary | ios::ate);
	if (!myfile.is_open()) {
		cout << "failed to open";
		return;
	}
...
int Intel_8080::execute(Intel_8080* i8080) {
	...
	//cout << hex << setw(4) << setfill('0') << (int)PC << ": ";
	//cout << hex << setw(2) << setfill('0') << (int)RAM[PC] << endl;
	...
	if (instruction == 0xD9) { //undocumented RET
		//START--RET
		PC = RAM[SP++];
		PC |= ((uint16_t)RAM[SP++] << 8);
		cycles += 10;
	...
	else if (instruction == 0x08 || instruction == 0x10 || instruction == 0x18 || instruction == 0x20 || instruction == 0x28 || instruction == 0x30 || instruction == 0x38) { //undocumented NOP
		//START--NOP
		PC += 1;
		cycles += 4;
		//END--NOP
